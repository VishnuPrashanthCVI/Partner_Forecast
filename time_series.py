...
class TimeSeries:
	...
	#fit prophet class and add 3 months
	...
	def average_time_error(self,dt):
		orig_mean = self.data.Revenue.mean()
		pred_mean = dt.Pred.mean()
		error = mse(self.data.Revenue,dt.Pred)
		values = [orig_mean, pred_mean, error]
		keys =  ['orig_mean', 'pred_mean', 'mean_squared_error']
