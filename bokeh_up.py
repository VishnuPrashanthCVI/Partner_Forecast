...
table_source.data=ColumnDataSource.from_df(metrics_up)
...
metric = DataTable(source=table_source, columns=columns, width=800, row_headers=True)'''
...
p1 = figure(title="Prior Months Revenue", plot_width=800, plot_height=500)
p1.line("M","R", source=timesource)
p1.title.text_font_size = "25px"
p1.title.align = "center"
#build drop down box
IDX = Select(value=partners_up[0], options=partners_up, title='Partners')
...
show(layout)
curdoc().add_root(layout)
curdoc().title = "Top Ten Partners"'''
'''bokeh html --show bokeh_up.py 
