...
def time_data(partner, d):
	...
	data = data.dropna()
	return data
data=time_data(partners_down[0], datadown)
...
p1.yaxis.axis_label='Revenue In Thousands'
...
fdata=feature_data(partners_down[0], features_down)
...
#build drop down box
IDX = Select(value=partners_down[0], options=partners_down, title='Select Partner')

def IDX_update(attribute, old, new):
	partner = IDX.value
	data = time_data(partner,datadown)
...
curdoc().title = "Bottom Twenty Partners"
'''bokeh serve --show bokeh_down.py 
